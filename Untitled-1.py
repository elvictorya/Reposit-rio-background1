...
# =============================================================================
# Análise via PyROOT (alternativa)
# =============================================================================

def analyze_pyroot(input_file: str, tree_name: str = "Delphes"):
    """Alternativa usando PyROOT diretamente."""
    print(f"\n[PyROOT] Lendo: {input_file}  (tree: {tree_name})")
    ...
    counters = {"I": 0, "II": 0, "III": 0, "IV": 0}

    for evt in chain:
        jets     = evt.Jet
        met_arr  = evt.MissingET
        electrons= evt.Electron
        muons    = evt.Muon

        # --- Corte I: >= 1 b-jet ---
        n_bjet = sum(
            1 for j in jets
            if j.PT > 20.0 and abs(j.Eta) < 2.5 and j.BTag
        )
        if n_bjet < 1:
            continue
        counters["I"] += 1
        ...
        # --- Corte II: MET > 20 GeV ---
        met_val = met_arr[0].MET if met_arr.GetEntries() > 0 else 0.0
        if met_val <= 20.0:
            continue
        counters["II"] += 1

        # --- Corte III: Exatamente 2 léptons SS ---
        leps = []
        for el in electrons:
            if el.PT > 10.0 and abs(el.Eta) < 2.5:
                leps.append({"pt": el.PT, "eta": el.Eta,
                             "phi": el.Phi, "charge": el.Charge, "mass": 0.0})
        for mu in muons:
            if mu.PT > 10.0 and abs(mu.Eta) < 2.5:
                leps.append({"pt": mu.PT, "eta": mu.Eta,
                             "phi": mu.Phi, "charge": mu.Charge, "mass": 0.10566})

        if len(leps) != 2:
            continue
        charges = [l["charge"] for l in leps]
        if not (all(c > 0 for c in charges) or all(c < 0 for c in charges)):
            continue
        counters["III"] += 1
        ...
        v0.SetPtEtaPhiM(l0["pt"], l0["eta"], l0["phi"], l0["mass"])
        v1.SetPtEtaPhiM(l1["pt"], l1["eta"], l1["phi"], l1["mass"])
        mll = (v0 + v1).M()
        if mll <= 100.0:
            continue
        counters["IV"] += 1
    ...
    for k in ["I", "II", "III", "IV"]:
        eff = counters[k] / N_total if N_total > 0 else 0
        print(f"{'(' + k + ')':<10} {counters[k]:>10}  {eff:>10.3f}")
    print("=" * 55)
    print("\nValores esperados (Tabela I, W±W±jj):")
    print("  (I) = 0.11 | (II) = 0.11 | (III) = 0.03 | (IV) = 0.02")
...
def main():
    parser = argparse.ArgumentParser(
        description="Aplica cortes cinemáticos (Eq. 5) e calcula eficiências para W±W±jj"
    )
    parser.add_argument(
        "--input", "-i",
        required=True,
        help="Caminho para o arquivo ROOT do Delphes (ex: tag_1_delphes_events.root)"
    )
    parser.add_argument(
        "--tree", "-t",
        default="Delphes",
        help="Nome da TTree no arquivo ROOT (padrão: Delphes)"
    )
    parser.add_argument(
        "--backend",
        choices=["uproot", "pyroot", "auto"],
        default="auto",
        help="Backend de leitura do ROOT (padrão: auto)"
    )
    args = parser.parse_args()

    backend = args.backend
    if backend == "auto":
        backend = "uproot" if USE_UPROOT else "pyroot"

    print("=" * 55)
    print(" ANÁLISE DE CORTES CINEMÁTICOS — W±W±jj")
    print(" (reproduzindo última coluna da Tabela I)")
    print("=" * 55)

    if backend == "uproot":
        if not USE_UPROOT:
            raise ImportError("uproot não instalado. Use: pip install uproot awkward vector")
        analyze_uproot(args.input, args.tree)
    else:
        if not USE_ROOT:
            raise ImportError("PyROOT não disponível.")
        analyze_pyroot(args.input, args.tree)
